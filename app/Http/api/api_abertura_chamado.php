<?php
    ...
    function configuraDisparoEmail($settaNovoBug){
      ...
      $mensagem = "<p>Olá! Seu chamado de suporte foi aberto. O número de identificação desse ticket é #".$settaNovoBug[0]['id_chamado']."</p>
      ...
      <p> O prazo para a solução é estimada em ".$settaNovoBug[0]['prazo']." horas </p>
      <p> Atenciosamente, </p>
      <p> Suporte Aquicob </p>"; 
  ...
  /*REGRA DE NEGOCIO BUG*/
    // o calculo de data estimada de entrega é feita no front, por ajax
    ...
    /*REQUESTS ORCAMENTO*/
      ...
      if(!isset($total_horas) || empty($total_horas)){
        $settaNovoBug = $novoBug->setNovoBug($id_cliente,$envolvidos,$id_solicitante,$titulo,$descricao,$data_entrega_estimada,$produto,$modulo,$tela, $direcionamento, $prioridade, $prazo); // novo bug
      }else{
    ...
    if ( isset( $_FILES["fileToUpload"] ) && !empty( $_FILES["fileToUpload"]["name"] ) ) {
      if ( is_uploaded_file( $_FILES["fileToUpload"]["tmp_name"] ) && $_FILES["fileToUpload"]["error"] === 0 ) {
        $uploadOk = 1;
        // RECUPERA A EXTENSÃO DO ARQUIVO ANEXADO.
